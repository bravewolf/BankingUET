...
/* JavaScript content from js/AboutPage.js in folder common */
currentPage={};

currentPage.init = function() {
	WL.Logger.debug("AboutPage :: init");
...
currentPage.back = function(){
	WL.Logger.debug("AboutPage :: back");
...
function onlineAuthentication() {
	 ...
	 */
	if (WL.Client.isUserAuthenticated("OfflineRealm")) {
		/*
	...
	// If logging-in while offline and Internet connectivity is then available, login to the realm without showing the login form.
		...
		OfflineRealmChallengeHandler.submitAdapterAuthentication(invocationData, {onSuccess:onlineAuthenticationSuccess, onFailure:onlineAuthenticationFailure });
	} else {
...
function logout() {
	...
	WL.Device.getNetworkInfo(
		function (networkInfo) {
			if (networkInfo.isNetworkConnected == "true") {
				WL.Client.logout("OfflineRealm", {onSuccess:displayUnsecuredArea, onFailure:displayUnsecuredArea});
			}
		}
	);
